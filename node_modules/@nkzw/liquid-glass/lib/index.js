...
const fragmentShaders = { liquidGlass: (uv) => {
	const ix = uv.x - .5;
	const iy = uv.y - .5;
	const distanceToEdge = roundedRectSDF(ix, iy, .3, .2, .6);
	const displacement = smoothStep(.8, 0, distanceToEdge - .15);
	const scaled = smoothStep(0, 1, displacement);
	return texture(ix * scaled + .5, iy * scaled + .5);
} };
var ShaderDisplacementGenerator = class {
	...
	updateShader(mousePosition) {
		...
		for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
			...
			const r = smoothedDx / maxScale + .5;
			const g = smoothedDy / maxScale + .5;
...
const GlassFilter = ({ aberrationIntensity, displacementScale, id, mode, shaderMapUrl }) => /* @__PURE__ */ jsx("svg", {
	"aria-hidden": "true",
	height: "100%",
	style: { position: "absolute" },
	width: "100%",
	children: /* @__PURE__ */ jsxs("defs", { children: [/* @__PURE__ */ jsxs("radialGradient", {
		cx: "50%",
		cy: "50%",
		id: `${id}-edge-mask`,
		r: "50%",
		...
		children: [
			...
			/* @__PURE__ */ jsx("feComponentTransfer", {
				in: "EDGE_INTENSITY",
				result: "EDGE_MASK",
				children: /* @__PURE__ */ jsx("feFuncA", {
					tableValues: `0 ${aberrationIntensity * .05} 1`,
					type: "discrete"
				})
			}),
			...
			/* @__PURE__ */ jsx("feDisplacementMap", {
				...
				scale: displacementScale * ((mode === "shader" ? 1 : -1) - aberrationIntensity * .05),
				xChannelSelector: "R",
				yChannelSelector: "B"
			}),
			/* @__PURE__ */ jsx("feColorMatrix", {
			...
			/* @__PURE__ */ jsx("feDisplacementMap", {
				...
				scale: displacementScale * ((mode === "shader" ? 1 : -1) - aberrationIntensity * .1),
				xChannelSelector: "R",
				yChannelSelector: "B"
			}),
			/* @__PURE__ */ jsx("feColorMatrix", {
			...
			/* @__PURE__ */ jsx("feGaussianBlur", {
				...
				stdDeviation: Math.max(.1, .5 - aberrationIntensity * .1)
			}),
...
const GlassContainer = forwardRef(({ aberrationIntensity = 2, blurAmount = 12, borderRadius = 999, children, className = "", displacementScale = 25, glassSize, mode = "standard", onClick, onMouseDown, onMouseEnter, onMouseLeave, onMouseUp, overLight = false, padding = "24px 32px", saturation = 180, style }, ref) => {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsx(GlassFilter, {
			aberrationIntensity,
			displacementScale,
			id: filterId,
			mode,
...
function LiquidGlass({ aberrationIntensity = 2, blurAmount = .0625, borderRadius = 999, children, className = "", displacementScale = 70, elasticity = .15, globalMousePos: externalGlobalMousePos, mode = "standard", mouseContainer = null, mouseOffset: externalMouseOffset, onClick, overLight = false, padding = "24px 32px", saturation = 140, style }) {
	...
	const calculateDirectionalScale = useCallback(() => {
		...
		const scaleX = 1 + Math.abs(normalizedX) * stretchIntensity * .3 - Math.abs(normalizedY) * stretchIntensity * .15;
		const scaleY = 1 + Math.abs(normalizedY) * stretchIntensity * .3 - Math.abs(normalizedX) * stretchIntensity * .15;
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx(GlassContainer, {
				aberrationIntensity,
				blurAmount,
				borderRadius,
				className,
				displacementScale: overLight ? displacementScale * .5 : displacementScale,
				glassSize,
				mode,
				mouseOffset,
				onClick,
			...
			glassSize.height > 0 && glassSize.width > 0 && /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsx("span", { style: {
				position: "absolute",
				...transformStyle,
				background: `linear-gradient(
          ${135 + mouseOffset.x * 1.2}deg,
          rgba(255, 255, 255, 0.0) 0%,
          rgba(255, 255, 255, ${.12 + Math.abs(mouseOffset.x) * .008}) ${Math.max(10, 33 + mouseOffset.y * .3)}%,
          rgba(255, 255, 255, ${.4 + Math.abs(mouseOffset.x) * .012}) ${Math.min(90, 66 + mouseOffset.y * .4)}%,
          ...
          ${135 + mouseOffset.x * 1.2}deg,
          rgba(255, 255, 255, 0.0) 0%,
          rgba(255, 255, 255, ${.32 + Math.abs(mouseOffset.x) * .008}) ${Math.max(10, 33 + mouseOffset.y * .3)}%,
          rgba(255, 255, 255, ${.6 + Math.abs(mouseOffset.x) * .012}) ${Math.min(90, 66 + mouseOffset.y * .4)}%,
          rgba(255, 255, 255, 0.0) 100%
        )`,
				borderRadius,
				boxShadow: "0 0 0 0.5px rgba(255, 255, 255, 0.5) inset, 0 1px 3px rgba(255, 255, 255, 0.25) inset, 0 1px 4px rgba(0, 0, 0, 0.35)",
			...
			Boolean(onClick) && /* @__PURE__ */ jsxs(Fragment, { children: [
				/* @__PURE__ */ jsx("div", { style: {
					...
					height: glassSize.height,
					mixBlendMode: "overlay",
					opacity: isHovered || isActive ? .5 : 0,
					pointerEvents: "none",
				...
				/* @__PURE__ */ jsx("div", { style: {
					...
					height: glassSize.height,
					mixBlendMode: "overlay",
					opacity: isHovered ? .4 : isActive ? .8 : 0,
					pointerEvents: "none",
					transition: "all 0.2s ease-out",
					width: glassSize.width + 1
				} })
