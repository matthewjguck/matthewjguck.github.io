...
const Shuffle = ({
  ...
  const hoverHandlerRef = useRef<((this: Element, ev: MouseEvent) => any) | null>(null);
      ...
      const build = () => {
        ...
        const measureVerticalCell = (node: Element, lineBoxHeight: number) => {
          ...
          if (metricsContext) {
            ...
            const sample = `${(node as HTMLElement).textContent || 'M'}${scrambleCharset}`;
            const metrics = metricsContext.measureText(sample);
            const ascent = (metrics as any).fontBoundingBoxAscent;
            const descent = (metrics as any).fontBoundingBoxDescent;
          ...
          if (!fontHeight) {
            ...
            probe.textContent = `${(node as HTMLElement).textContent || 'M'}${scrambleCharset}`;
        ...
        chars.forEach(ch => {
          const parent = (ch as HTMLElement).parentElement;
          if (!parent) return;
          ...
          (ch as HTMLElement).setAttribute('data-orig', '1');
